...
export default function Home() {
  ...
  const handleChangePassword = (e) => {
    ...
    setPassMessage('✅ ¡Credenciales actualizadas en la nube!');
    setNewPass('');
    setConfirmPass('');
    setNewRecoveryMail('');
  };
  ...
  const [newMovement, setNewMovement] = useState({ 
    ...
    convertirAPlazo: false,
    plazoDias: 5,
    agendarAudiencia: false, // <-- Salta automáticamente a audiencias
  ...
  const handleAskAI = (e) => {
    ...
    setTimeout(() => {
      ...
      } else if (qLower.includes('audiencia') || qLower.includes('calendario')) {
        aiResponse = `Dr., tiene ${hearings.length} audiencia(s) agendada(s) en la sección "Audiencias y Calendario".`;
      } else {
        aiResponse = `Dr., analizando su consulta ("${userMsg}") frente a las fuentes normativas y expedientes del estudio: Se sugiere revisar la solapa correspondiente para un cotejo exacto.`;
      }
  ...
  const syncWithCloud = async (key, value) => {
    ...
    } catch (e) { console.error('Error sincronizando con nube:', e); }
  };
  ...
  // AUTOMATIZACIÓN DESDE MOVIMIENTO DE EXPEDIENTE (SALTA A AUDIENCIAS)
    ...
    return (
      <div className="flex h-screen bg-zinc-950 text-zinc-100 font-sans items-center justify-center p-4">
        <div className="w-full max-w-sm bg-zinc-900 border border-zinc-800 p-8 rounded-2xl shadow-2xl text-center space-y-6">
          ...
          <p className="text-[10px] text-zinc-600">Sesión protegida.</p>
        </div>
      </div>
    );
  }
        ...
        <div>
          ...
          <nav className="p-3 space-y-1">
            {[
              ...
              { id: 'audiencias', label: 'Audiencias y Calendario', icon: '📅' }, // <-- NOMBRE CORREGIDO
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-zinc-950 relative">
          ...
          {selectedCaseId && selectedCaseData ? (
                    ...
                    <div key={m.id} className="p-3 bg-zinc-950 border border-zinc-800 rounded text-xs space-y-2">
                      {editingMovementId === m.id ? (
                        <div className="space-y-2">
                          <input 
                            type="text" value={editMovementForm.title} 
                          ...
                          />
                          <input 
                            type="date" value={editMovementForm.date} 
                          ...
                          <div className="flex gap-2">
                            <button onClick={() => handleSaveEditMovement(m.id)} className="bg-emerald-600 text-white font-bold px-3 py-1 rounded">Guardar</button>
                        ...
                        <>
                          <div className="flex justify-between items-center font-bold text-zinc-200">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <button onClick={() => handleStartEditMovement(m)} className="text-zinc-400 hover:text-white px-2 py-0.5 bg-zinc-900 rounded border border-zinc-800">✏️</button>
                              <button onClick={() => handleDeleteMovement(m.id)} className="text-red-400 hover:text-red-300 px-2 py-0.5 bg-red-500/10 rounded border border-red-500/20">🗑️</button>
                            </div>
                          </div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl space-y-3">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => startEditingFiscal(selectedFiscalData)} className="bg-zinc-800 hover:bg-zinc-700 text-orange-400 font-bold text-xs px-3 py-1.5 rounded border border-zinc-700">
                      ✏️ Editar Fechas
                    </button>
                  </div>
                </div>
            ...
            <>
              ...
              {activeTab === 'asistente_ia' && (
                <div className="flex h-[calc(100vh-100px)] gap-4 relative z-10">
                  <div className="w-64 bg-zinc-900 border border-zinc-800 rounded-2xl flex flex-col p-3 shrink-0">
                    <button onClick={handleCreateNewChat} className="w-full bg-orange-500 text-black font-bold text-xs py-2.5 px-3 rounded-xl mb-3">
                      ✨ Nuevo Chat Legal
                    </button>
                    ...
                    <div className="pt-2 border-t border-zinc-800 mt-2">
                      <button onClick={() => setShowSourceModal(true)} className="w-full bg-zinc-950 text-zinc-300 font-bold text-xs py-2 rounded-xl border border-zinc-800">
                        📚 Cargar Leyes ({customSources.length})
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl flex flex-col overflow-hidden shadow-2xl">
                    <div className="p-4 border-b border-zinc-800 bg-zinc-950 flex justify-between items-center">
                      <h3 className="text-xs font-bold text-orange-500 uppercase">Asistente IA Jurídico (Modo Gemini Pro)</h3>
                    </div>
                    <div className="flex-1 overflow-y-auto p-5 space-y-4">
                      ...
                      {isAiLoading && <div className="text-zinc-400 text-xs italic animate-pulse">Buscando en leyes...</div>}
                    </div>
                    <form onSubmit={handleAskAI} className="p-4 bg-zinc-950 border-t border-zinc-800 flex items-center gap-2">
                      ...
                      <input type="text" placeholder="Pregunte sobre plazos, leyes o expedientes..." value={aiQuery} onChange={e => setAiQuery(e.target.value)} className="flex-1 bg-zinc-900 border border-zinc-800 p-3.5 rounded-xl text-xs text-white outline-none focus:border-orange-500" />
                  ...
                  {showSourceModal && (
                    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
                      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-xl rounded-2xl p-6 space-y-5">
                        <div className="flex justify-between items-center border-b border-zinc-800 pb-3">
                          <h3 className="text-sm font-bold text-orange-500 uppercase">📚 Fuentes y Leyes para la IA</h3>
                          <button onClick={() => setShowSourceModal(false)} className="text-zinc-400 text-xs">✕ Cerrar</button>
                        </div>
                        <form onSubmit={handleAddCustomSource} className="space-y-3">
                          <input type="text" placeholder="Título de la Ley (ej. Ley 24.522)" value={newSourceTitle} onChange={e => setNewSourceTitle(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 p-3 rounded-xl text-xs text-white" />
                          <textarea placeholder="Texto de la ley..." value={newSourceContent} onChange={e => setNewSourceContent(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 p-3 rounded-xl text-xs text-white h-28" />
                          <button type="submit" className="w-full bg-orange-500 text-black font-bold text-xs py-3 rounded-xl">Guardar Ley</button>
                        </form>
                      </div>
                    </div>
                  )}
              ...
              {/* MÓDULO DE AUDIENCIAS Y CALENDARIO (CON CALENDARIO MENSUAL VISUAL EN PANTALLA) */}
              {activeTab === 'audiencias' && (
                <div className="space-y-6 relative z-10">
                  <form onSubmit={handleAddHearingDirect} className="bg-zinc-900 border border-zinc-800 p-5 rounded-xl space-y-4 shadow-xl">
                    ...
                    <textarea 
                      placeholder="Observaciones o instrucciones para la audiencia..."
                  ...
                  {/* CALENDARIO MENSUAL VISUAL + LISTADO */}
                        ...
                        {hearings.map(h => (
                          <div key={h.id} className="p-4 bg-zinc-950 border border-zinc-800 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-3 text-xs">
                            ...
                            <div className="flex items-center gap-2 self-end md:self-center">
                              <button onClick={() => toggleHearingStatus(h.id)} className={`px-3 py-1.5 rounded font-bold text-[10px] ${h.status === 'REALIZADA' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
                                {h.status}
                              </button>
                              <button onClick={() => deleteHearing(h.id)} className="bg-red-500/10 text-red-400 p-2 rounded">🗑️</button>
                            </div>
                          </div>
                        ))}
              ...
              {activeTab === 'expedientes' && (
                <div className="space-y-6 relative z-10">
                  <form onSubmit={(e) => {
                    ...
                    <h3 className="text-xs font-bold text-orange-500 uppercase">+ Agregar Nuevo Expediente</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                      <input type="text" placeholder="Nº de Expediente" value={newCase.number} onChange={e => setNewCase({...newCase, number: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white" />
                    ...
                    {cases.map(c => (
                      <div key={c.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => deleteCase(c.id)} className="bg-red-500/10 text-red-400 px-3 py-1.5 rounded text-xs font-bold">🗑️</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {activeTab === 'movimientos' && (
                <div className="space-y-6 relative z-10">
                  <div className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl space-y-3">
                    <h4 className="text-xs font-bold text-orange-500 uppercase">Historial de Movimientos Generales</h4>
                    <div className="space-y-2">
                      {movements.map(m => (
              ...
              {activeTab === 'plazos' && (
                <div className="space-y-6 relative z-10">
                  <form onSubmit={handleAddDeadline} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                      <select value={newDeadline.caseId} onChange={e => setNewDeadline({...newDeadline, caseId: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white">
                        <option value="">Seleccionar Expediente...</option>
                        {cases.map(c => <option key={c.id} value={c.id}>{c.number} - {c.caratula}</option>)}
                      </select>
                      <input type="text" placeholder="Descripción del Plazo" value={newDeadline.title} onChange={e => setNewDeadline({...newDeadline, title: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white" />
                      <input type="date" value={newDeadline.dueDate} onChange={e => setNewDeadline({...newDeadline, dueDate: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white" />
                      <input type="number" placeholder="Días" value={newDeadline.days} onChange={e => setNewDeadline({...newDeadline, days: parseInt(e.target.value)})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white" />
                    </div>
                    <button type="submit" className="bg-orange-500 text-black font-bold text-xs px-4 py-2 rounded">Registrar</button>
                  </form>
                  <div className="space-y-3">
                    {deadlines.map(d => (
              ...
              {activeTab === 'tareas' && (
                <div className="space-y-6 relative z-10">
                  <form onSubmit={handleAddTask} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                      <input type="text" placeholder="Descripción" value={newTask.title} onChange={e => setNewTask({...newTask, title: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white md:col-span-2" />
                      <select value={newTask.priority} onChange={e => setNewTask({...newTask, priority: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white">
              ...
              {activeTab === 'clientes' && (
                <div className="space-y-6 relative z-10">
                  <form onSubmit={handleAddClient} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl space-y-3">
                    ...
                    <input type="text" placeholder="Nombre completo" value={newClient.name} onChange={e => setNewClient({...newClient, name: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white text-xs" />
                    <button type="submit" className="bg-orange-500 text-black font-bold text-xs px-4 py-2 rounded">Guardar</button>
                  </form>
                  <div className="space-y-2">
                    {clients.map(cli => (
                      <div key={cli.id} className="bg-zinc-900 border border-zinc-800 p-3 rounded-xl flex justify-between items-center text-xs">
                        <div>
                          <h4 className="font-bold text-white">{cli.name}</h4>
                          <p className="text-zinc-500">{cli.role} • {cli.email || 'Sin mail'}</p>
                        </div>
                        <button onClick={() => deleteClient(cli.id)} className="bg-red-500/10 text-red-400 p-2 rounded">🗑️</button>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  {procuracionSubTab === 'titulos' && (
                    <div className="space-y-4">
                      <form onSubmit={handleAddFiscalCase} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl space-y-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                          <input type="text" placeholder="Contribuyente" value={newFiscalCase.contribuyente} onChange={e => setNewFiscalCase({...newFiscalCase, contribuyente: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white" />
                          <input type="text" placeholder="Nº de Liquidación" value={newFiscalCase.nroLiquidacion} onChange={e => setNewFiscalCase({...newFiscalCase, nroLiquidacion: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white" />
                          <input type="text" placeholder="Monto ($)" value={newFiscalCase.monto} onChange={e => setNewFiscalCase({...newFiscalCase, monto: e.target.value})} className="bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white" />
                        </div>
                      ...
                      <div className="space-y-3">
                        {fiscalCases.map(fc => (
                          <div key={fc.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl text-xs space-y-2 flex justify-between items-center">
                            <div>
                              <span className="bg-orange-500/10 text-orange-400 font-mono px-2 py-0.5 rounded mr-2">Liq: {fc.nroLiquidacion}</span>
                              <span className="font-bold text-white text-sm">{fc.contribuyente}</span>
                              <p className="text-zinc-500 mt-0.5">Monto: {fc.monto}</p>
                            </div>
                  ...
                  {procuracionSubTab === 'plantillas' && (
                    <div className="space-y-4">
                      <form onSubmit={handleAddTemplate} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl space-y-3">
                        ...
                        <input type="text" placeholder="Título de la plantilla" value={newTemplateTitle} onChange={e => setNewTemplateTitle(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 p-2.5 rounded text-white text-xs" />
                        <button type="submit" className="bg-orange-500 text-black font-bold text-xs px-4 py-2 rounded">Guardar Plantilla</button>
                      </form>
                      <div className="space-y-2">
                        {templates.map(t => (
                          <div key={t.id} className="bg-zinc-900 border border-zinc-800 p-3 rounded-xl flex justify-between items-center text-xs">
                            <div>
                              <h4 className="font-bold text-white">{t.title}</h4>
                              <p className="text-zinc-500">Categoría: {t.category} • Archivo: {t.fileName}</p>
                            </div>
                            <button onClick={() => deleteTemplate(t.id)} className="bg-red-500/10 text-red-400 p-2 rounded">🗑️</button>
                          </div>
                        ))}
                      </div>
                    </div>
