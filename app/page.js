                      ...
                      <form onSubmit={handleAddCautelar} className={`border p-5 rounded-xl space-y-4 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs">
                          ...
                          <select 
                            ...
                            className={`border p-2.5 rounded outline-none focus:border-orange-500 ${isDarkMode ? 'bg-zinc-950 border-zinc-800 text-white' : 'bg-zinc-50 border-zinc-300 text-zinc-900'}`}
                          >
                            <option value="SOJ (Bancario)">Tipo: SOJ (Bancario / Fondos)</option>
                            <option value="Inhibición General de Bienes">Tipo: Inhibición General de Bienes</option>
                            <option value="Embargo Automotor">Tipo: Embargo Automotor (Registro Propiedad)</option>
                            <option value="Embargo Inmobiliario">Tipo: Embargo Inmobiliario</option>
                          </select>
                          <input 
                          ...
                          />
                          <input 
                            type="text" placeholder="Monto trabado ($)" 
                        ...
                        {cautelares.map(c => (
                          <div key={c.id} className={`border p-4 rounded-xl flex justify-between items-center text-xs ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}>
                            <div>
                              ...
                              <p className="text-[11px] text-zinc-500 mt-0.5">Fecha traba: {formatDateToArg(c.fecha)} • Monto: <strong className="text-emerald-500">{c.montoEmbargo}</strong></p>
                            </div>
                  ...
                  {procuracionSubTab === 'honorarios' && (
                    <div className="space-y-6">
                      <form onSubmit={handleAddHonorario} className={`border p-5 rounded-xl space-y-4 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}>
                        <h4 className="text-xs font-bold text-orange-500 uppercase">+ Registrar Cobro de Honorarios o Gasto Procesal (Procuración)</h4>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs">
                          <select 
                          ...
                          >
                            ...
                            <option value="GASTO_JUDICIAL">Tipo: Gasto Judicial / Tasa / Oficio</option>
                          </select>
                          <input 
                        ...
                        <input 
                          type="text" placeholder="Concepto detallado (ej. Regulación de honorarios primera etapa)" 
                          value={newHonorario.concepto} onChange={e => setNewHonorario({...newHonorario, concepto: e.target.value})}
                          className={`w-full border p-2.5 rounded outline-none focus:border-orange-500 text-xs ${isDarkMode ? 'bg-zinc-950 border-zinc-800 text-white' : 'bg-zinc-50 border-zinc-300 text-zinc-900'}`}
                        />
                        <button type="submit" className="bg-orange-500 text-black font-bold text-xs px-4 py-2.5 rounded hover:bg-orange-400">
                          Registrar en Caja Estudio
                        </button>
                      </form>

                      <div className="space-y-3">
                        <h4 className="text-xs font-bold text-orange-500 uppercase">Movimientos Registrados en Procuración</h4>
                        {honorariosProcuracion.map(h => (
                          <div key={h.id} className={`border p-4 rounded-xl flex justify-between items-center text-xs ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}>
                            <div>
                              <span className="bg-orange-500/10 text-orange-500 font-bold px-2 py-0.5 rounded text-[10px] mr-2">{h.tipoIngreso}</span>
                              <strong className={isDarkMode ? 'text-white' : 'text-zinc-900'}>{h.concepto}</strong>
                              <p className="text-[11px] text-zinc-500 mt-0.5">Liq Ref: {h.nroLiquidacion} • Fecha: {formatDateToArg(h.fecha)} • Monto: <strong className="text-orange-500">${h.monto}</strong></p>
                            </div>
                ...
                <div className="space-y-6 relative z-10">
                  <div className={`border p-6 rounded-xl space-y-4 shadow-xl ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-orange-500 uppercase">Módulo de Emisión de Recibos Oficiales (Original y Duplicado)</h3>
                        <p className="text-xs text-zinc-400">Emita recibos por anticipos, fondo fijo, regulación de honorarios o gastos con conversión automática a letras en pesos argentinos.</p>
                      </div>
                    </div>
                    ...
                    <form onSubmit={(e) => {
                      ...
                      const tipoFinal = reciboForm.tipoRecibo === 'OTRO' ? (reciboForm.customTipo.trim() || 'Recibo Oficial') : reciboForm.tipoRecibo;
                      let referenciaAsociada = 'General / Estudio';
                      
                      if (reciboForm.asociacionTipo === 'expediente' && reciboForm.causaId) {
                        const cFound = cases.find(c => c.id === reciboForm.causaId);
                        if (cFound) referenciaAsociada = `Expediente Nº ${cFound.number} (${cFound.caratula})`;
                      } else if (reciboForm.asociacionTipo === 'fiscal' && reciboForm.fiscalId) {
                        const fFound = fiscalCases.find(fc => fc.id === reciboForm.fiscalId);
                        if (fFound) referenciaAsociada = `Título Fiscal Liq. ${fFound.nroLiquidacion} - ${fFound.contribuyente}`;
                      }
                      ...
                      const receiptHtml = `
                        ...
                        <head>
                          <meta charset="utf-8">
                          <title>${tipoFinal} - Estudio Jurídico MM</title>
                          <style>
                            body { font-family: Arial, sans-serif; color: #000; margin: 0; padding: 20px; font-size: 12px; }
                            .recibo-container { border: 2px dashed #333; padding: 25px; margin-bottom: 30px; position: relative; background: #fff; }
                            .header { display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #f97316; padding-bottom: 15px; margin-bottom: 15px; }
                        ...
                        <body>
                          ...
                          <div class="recibo-container">
                            ...
                            <div class="details-grid">
                              ...
                              <div><strong>Referencia:</strong> ${referenciaAsociada}</div>
                            </div>
                            ...
                            <div class="concepto-box">
                              <strong>En concepto de:</strong> ${reciboForm.conceptoDetallado.trim() || tipoFinal} (${referenciaAsociada}).
                            </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-zinc-500 font-bold block mb-1">Tipo de Recibo:</label>
                          <select 
                          ...
                          >
                            ...
                            <option value="Entrega de dinero a cuenta">Entrega de dinero a cuenta</option>
                            <option value="Reintegro de viáticos / Tasas">Reintegro de viáticos / Tasas</option>
                        ...
                        <div>
                          <label className="text-zinc-500 font-bold block mb-1">Nombre del Pagador / Cliente:</label>
                          <input 
                            type="text"
                            placeholder="Ej. Roberto García / Razón Social"
                        ...
                        <div>
                          <label className="text-zinc-500 font-bold block mb-1">Asociar a:</label>
                          ...
                          {reciboForm.asociacionTipo === 'expediente' ? (
                            ...
                            >
                              <option value="">Seleccionar Expediente...</option>
                              {cases.map(c => <option key={c.id} value={c.id}>Expediente Nº {c.number} - {c.caratula}</option>)}
                            </select>
                          ) : (
                            <select 
                        ...
                        <div>
                          <label className="text-zinc-500 font-bold block mb-1">Monto en Pesos ($):</label>
                          <input 
                            type="text"
                          ...
                          />
                          {reciboForm.monto && (
                            <p className="text-[10px] text-orange-500 font-bold italic mt-1">{numeroALetras(reciboForm.monto)}</p>
                          )}
                        </div>
                      </div>

                      <div>
                        <label className="text-zinc-500 font-bold block mb-1">Concepto Detallado (Opcional):</label>
                        <textarea 
                          placeholder="Describa brevemente el motivo del pago..."
                      ...
                      <button type="submit" className="bg-orange-500 text-black font-bold px-6 py-3 rounded-lg shadow-lg shadow-orange-500/20 hover:bg-orange-400">
                        🖨️ Generar e Imprimir Recibo Oficial (Original y Duplicado)
                      </button>
                    </form>
                  </div>
                </div>
              ...
              {activeTab === 'finanzas' && (
                <div className="space-y-6 relative z-10">
                  <div className={`border p-5 rounded-xl space-y-4 ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}>
                    <div className="flex justify-between items-center">
                      <div>
                        <h3 className="text-sm font-bold text-orange-500 uppercase">💰 Caja General del Estudio y Tesorería</h3>
                        <p className="text-xs text-zinc-400">Control unificado de honorarios cobrados y gastos operativos. Seleccione múltiples registros para autosuma instantánea.</p>
                      </div>

                      <button 
                        onClick={() => {
                          if (selectedFinanceIds.length === 0) {
                            alert('Seleccione al menos un registro en la lista para calcular la autosuma.');
                            return;
                          }
                          const sum = honorariosProcuracion
                            .filter(h => selectedFinanceIds.includes(h.id))
                            .reduce((acc, curr) => acc + (parseFloat(curr.monto.replace(/[^0-9,.-]+/g, "").replace(",", ".")) || 0), 0);
                          alert(`🧮 Autosuma de los ${selectedFinanceIds.length} ítems seleccionados:\n\n$ ${sum.toLocaleString('es-AR')} (${numeroALetras(sum)})`);
                        }}
                        className="bg-orange-500 hover:bg-orange-400 text-black font-bold text-xs px-4 py-2 rounded shadow transition-all flex items-center gap-1.5"
                      >
                        <span>🧮 Calcular Autosuma ({selectedFinanceIds.length} sel.)</span>
                      </button>
                    </div>

                    <form onSubmit={handleAddDirectFinance} className={`p-4 rounded-xl border grid grid-cols-1 md:grid-cols-4 gap-3 text-xs ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
                      <input 
                        type="date"
                        value={newDirectFinance.fecha}
                        onChange={e => setNewDirectFinance({...newDirectFinance, fecha: e.target.value})}
                        className={`border p-2.5 rounded outline-none ${isDarkMode ? 'bg-zinc-900 border-zinc-700 text-white' : 'bg-white border-zinc-300 text-zinc-900'}`}
                      />
                      ...
                      <div className="flex flex-col gap-1">
                        <select 
                          ...
                          className={`border p-2.5 rounded outline-none ${isDarkMode ? 'bg-zinc-900 border-zinc-700 text-white' : 'bg-white border-zinc-300 text-zinc-900'}`}
                        >
                          <option value="HONORARIOS">Ingreso: Honorarios</option>
                          <option value="GASTO_JUDICIAL">Gasto: Judicial / Tasas</option>
                        ...
                        {newDirectFinance.tipoIngreso === 'OTRO' && (
                          <input 
                            ...
                            className={`border border-orange-500 p-2 rounded outline-none mt-1 ${isDarkMode ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-900'}`}
                            required
                          />
                        )}
                      </div>
                    ...
                    <div className="space-y-2">
                      {honorariosProcuracion.map(h => {
                        ...
                        return (
                          <div key={h.id} className={`p-3 rounded-lg border flex justify-between items-center text-xs transition-all ${isSelected ? 'border-orange-500 bg-orange-500/5' : (isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200')}`}>
                            <div className="flex items-center gap-3">
                              <input 
                                type="checkbox"
                                checked={isSelected}
                              ...
                              />
                              <div>
                                <span className={`font-bold px-2 py-0.5 rounded text-[9px] mr-2 ${h.tipoIngreso === 'HONORARIOS' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>{h.tipoIngreso}</span>
                                <strong className={isDarkMode ? 'text-white' : 'text-zinc-900'}>{h.concepto}</strong>
                                <p className="text-[10px] text-zinc-400 mt-0.5">Fecha: {formatDateToArg(h.fecha)} • Ref: {h.nroLiquidacion || 'General'}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              <span className="font-mono font-bold text-orange-500">${h.monto}</span>
                      ...
                      {honorariosProcuracion.length === 0 && (
                        <p className="text-xs text-zinc-500 italic p-4 text-center">No hay movimientos financieros registrados en caja.</p>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  <div className={`border p-6 rounded-xl space-y-4 shadow-xl ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-zinc-400">Resumen ejecutivo con todas las audiencias, plazos pendientes y tareas prioritarias del día.</p>
                        </div>
                      </div>
                      <button onClick={() => window.print()} className="bg-orange-500 text-black font-bold text-xs px-4 py-2 rounded shadow hover:bg-orange-400">
                    ...
                    <div className="space-y-4 text-xs">
                      <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
                        <h4 className="font-bold text-orange-500 uppercase mb-2">📅 Audiencias Agendadas ({hearings.filter(h => h.status === 'PENDIENTE').length})</h4>
                        {hearings.filter(h => h.status === 'PENDIENTE').length > 0 ? (
                          <ul className="list-disc pl-5 space-y-1">
                            {hearings.filter(h => h.status === 'PENDIENTE').map(h => (
                              <li key={h.id}><strong>{formatDateToArg(h.date?.split('T')[0])} {h.date?.split('T')[1]}</strong> — {h.title} ({h.tipoAudiencia} en {h.location})</li>
                            ))}
                          </ul>
                        ) : <p className="text-zinc-500 italic">No hay audiencias pendientes.</p>}
                      </div>

                      <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
                        <h4 className="font-bold text-orange-500 uppercase mb-2">⚡ Plazos Procesales Venciendo ({deadlines.filter(d => d.status === 'PENDIENTE').length})</h4>
                        {deadlines.filter(d => d.status === 'PENDIENTE').length > 0 ? (
                          <ul className="list-disc pl-5 space-y-1">
                            {deadlines.filter(d => d.status === 'PENDIENTE').map(d => (
                              <li key={d.id}><strong>Vence {formatDateToArg(d.dueDate)}</strong> ({d.days} días): {d.title}</li>
                            ))}
                          </ul>
                      ...
                      <div className={`p-4 rounded-lg border ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
                        <h4 className="font-bold text-orange-500 uppercase mb-2">✅ Tareas Prioritarias del Estudio ({tasks.filter(t => !t.completed).length})</h4>
                        {tasks.filter(t => !t.completed).length > 0 ? (
                          <ul className="list-disc pl-5 space-y-1">
                            {tasks.filter(t => !t.completed).map(t => (
                              <li key={t.id}><strong>[{t.priority}]</strong> {t.title}</li>
                            ))}
                          </ul>
                ...
                <div className="space-y-6 relative z-10">
                  <div className={`border p-6 rounded-xl space-y-4 shadow-xl ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-orange-500 uppercase">Contactos Útiles de Juzgados y Tribunales • Río Cuarto</h3>
                        <p className="text-xs text-zinc-400">Directorio oficial para gestiones procesales, mesas de entradas y despachos en la circunscripción judicial.</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
                      {[
                        { name: 'Juzgado Civil y Comercial 1ª Nominación', phone: '0358-4678000 (Int. 101)', email: 'jcivcom1-rc@justiciacordoba.gob.ar', dir: 'Palacio de Tribunales - Alvear y Pringles' },
                        { name: 'Juzgado Civil y Comercial 2ª Nominación', phone: '0358-4678000 (Int. 102)', email: 'jcivcom2-rc@justiciacordoba.gob.ar', dir: 'Palacio de Tribunales' },
                        { name: 'Juzgado Civil y Comercial 3ª Nominación', phone: '0358-4678000 (Int. 103)', email: 'jcivcom3-rc@justiciacordoba.gob.ar', dir: 'Palacio de Tribunales' },
                        { name: 'Juzgado Civil y Comercial 4ª Nominación', phone: '0358-4678000 (Int. 104)', email: 'jcivcom4-rc@justiciacordoba.gob.ar', dir: 'Palacio de Tribunales' },
                        { name: 'Fuero Laboral (Conciliación)', phone: '0358-4678000 (Int. 120)', email: 'jlab-rc@justiciacordoba.gob.ar', dir: 'Palacio de Tribunales' },
                        { name: 'Juzgado Fiscal / Procuración de Rentas', phone: '0358-4678000 (Int. 135)', email: 'jfiscal-rc@justiciacordoba.gob.ar', dir: 'Río Cuarto' }
                      ].map((j, idx) => (
                        <div key={idx} className={`p-3 rounded-lg border space-y-1 ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-zinc-50 border-zinc-200'}`}>
                          <strong className={isDarkMode ? 'text-white' : 'text-zinc-900'}>{j.name}</strong>
                          <p className="text-zinc-500">📍 {j.dir}</p>
                          <p className="text-zinc-500">📞 {j.phone}</p>
                          <p className="text-orange-500">✉️ {j.email}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="space-y-6 relative z-10">
                  <div className={`border p-6 rounded-xl space-y-6 shadow-xl ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                    <div className="flex items-center gap-3 border-b pb-4">
                      <span className="text-2xl">⚙️</span>
                      <div>
                        <h3 className="text-sm font-bold text-orange-500 uppercase">Configuración General del Estudio MM</h3>
                        <p className="text-xs text-zinc-400">Gestione correos electrónicos del equipo para Google Calendar, seguridad de acceso y resguardo de datos.</p>
                      </div>
                    </div>

                    <div className="space-y-4 text-xs">
                      <h4 className="font-bold text-orange-500 uppercase">📧 Correos Electrónicos del Equipo (Notificaciones Calendario)</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className={`pt-6 border-t space-y-4 text-xs ${isDarkMode ? 'border-zinc-800' : 'border-zinc-200'}`}>
                      <h4 className="font-bold text-orange-500 uppercase">🔒 Seguridad y Contraseña Universal en Nube</h4>
                      <form onSubmit={handleChangePassword} className="space-y-4 max-w-lg">
                        <div>
                          <label className="text-zinc-500 block mb-1 font-bold">Nueva Contraseña Universal:</label>
                          <input 
                            type="password"
                        ...
                        <div>
                          <label className="text-zinc-500 block mb-1 font-bold">Confirmar Nueva Contraseña:</label>
                          <input 
                            type="password"
                        ...
                        <div>
                          <label className="text-zinc-500 block mb-1 font-bold">Mail de Recuperación:</label>
                          <input 
                            type="email"
                        ...
                        {passMessage && (
                          <p className="font-bold p-2 rounded bg-orange-500/10 text-orange-500 border border-orange-500/20">{passMessage}</p>
                        )}

                        <button type="submit" className="bg-orange-500 text-black font-bold px-4 py-2.5 rounded shadow hover:bg-orange-400">
                          Actualizar Credenciales en la Nube
                        </button>
                      </form>
                    </div>

                    <div className={`pt-6 border-t space-y-3 text-xs ${isDarkMode ? 'border-zinc-800' : 'border-zinc-200'}`}>
                      <h4 className="font-bold text-orange-500 uppercase">💾 Copia de Seguridad y Resguardo (Descarga de Base Completa en TXT)</h4>
                      <p className="text-zinc-400">Descargue un archivo de texto estructurado con toda la base de datos de expedientes, clientes, movimientos y finanzas para resguardo offline.</p>
                      <button onClick={handleDownloadFullBackup} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-4 py-2.5 rounded shadow flex items-center gap-2">
